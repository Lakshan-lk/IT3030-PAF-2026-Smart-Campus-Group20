...
const AdminUserForm = () => {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      setForm(initialForm);
      setSuccess(created?.universityId ? `User created with ID ${created.universityId}` : 'User created successfully');
    } catch (mutationError) {
    ...
    <section className="bg-white/80 dark:bg-slate-800/60 backdrop-blur-sm rounded-2xl border border-slate-200/60 dark:border-slate-700/40 p-5">
      ...
      <div className="mt-4 rounded-xl bg-slate-50 dark:bg-slate-700/30 p-3 text-xs text-slate-500 dark:text-slate-400">
        <div className="flex items-start gap-2">
          ...
          <p>ID format: <span className="font-mono">UNI + year(2) + type(20/10) + sequence(1110...)</span>.</p>
        </div>
      </div>
    </section>
  );
